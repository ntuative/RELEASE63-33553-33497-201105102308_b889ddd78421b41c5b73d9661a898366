package com.sulake.core.utils
{
   ...
   public class LibraryLoaderQueue extends EventDispatcher implements IDisposable
   {
      ...
      protected static const const_1634:int = 4;
      ...
      {
         return this.var_471.length + this.var_285.length;
      }
      ...
      private function next() : void
      {
         ...
         if(!disposed)
         {
            while(this.var_285.length < const_1634 && this.var_471.length > 0)
            {
               _loc1_ = this.var_471.shift();
      ...
      private function removeLoader(param1:LibraryLoader) : void
      {
         ...
         try
         {
            index = this.var_471.indexOf(loader);
            if(index > -1)
            {
               this.var_471.splice(index,1);
            }
      ...
      private function isUrlInQueue(param1:String, param2:Boolean = true) : Boolean
      {
         ...
         if(!disposed)
         {
            ...
            for each(_loc3_ in this.var_471)
            {
               if(param2)
               {
