package com.sulake.habbo.friendlist.domain
{
   ...
   public class FriendCategories
   {
      ...
      {
         var _loc2_:int = !!param1.online ? int(param1.categoryId) : int(FriendCategory.const_414);
         ...
         try
         {
            ...
            for each(updatedFriend in p.updatedFriends)
            {
               ...
               if(currentlyOnline && !updatedFriend.online)
               {
                  ...
                  this.var_441.notifications.addOfflineNotification(updatedFriend.name,updatedFriend.realName);
               }
               if(!currentlyOnline && updatedFriend.online)
               {
                  this.var_441.messenger.setOnlineStatus(updatedFriend.id,updatedFriend.online);
                  this.var_441.view.setNewMessageArrived();
                  this.var_441.notifications.addOnlineNotification(updatedFriend.name,updatedFriend.figure,updatedFriend.realName);
               }
      ...
      private function updateCategories(param1:Array) : void
      {
         ...
         this.findCategory(FriendCategory.const_414).received = true;
         this.findCategory(FriendCategory.const_1209).received = true;
