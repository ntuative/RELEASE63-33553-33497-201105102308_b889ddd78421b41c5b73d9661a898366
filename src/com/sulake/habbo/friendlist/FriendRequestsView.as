package com.sulake.habbo.friendlist
{
   ...
   public class FriendRequestsView implements ITabView, IFriendRequestsView
   {
      ...
      private static const const_1924:String = "no_reqs_info";
      ...
      {
         this.var_1755 = IContainerButtonWindow(param1.findChildByName("accept_all_but"));
         this.var_1756 = IContainerButtonWindow(param1.findChildByName("reject_all_but"));
         this.var_1756.procedure = this.onDismissAllButtonClick;
         this.var_1755.procedure = this.onAcceptAllButtonClick;
         this.refreshButtons();
      }
      ...
      public function refreshRequestEntry(param1:FriendRequest) : void
      {
         ...
         {
            this._friendList.refreshIcon(_loc2_,"accept",true,this.method_6,param1.requestId);
      ...
      public function declineRequest(param1:int) : void
      {
         ...
         if(this._friendList && this._friendList.events)
         {
            this._friendList.events.dispatchEvent(new FriendRequestEvent(FriendRequestEvent.const_346,param1));
         }
      }
      ...
      private function onDismissAllButtonClick(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         {
            if(_loc4_.state != FriendRequest.const_401 && _loc4_.state != FriendRequest.const_423)
            {
               _loc4_.state = FriendRequest.const_423;
               this.refreshRequestEntry(_loc4_);
               if(this._friendList && this._friendList.events)
               {
                  this._friendList.events.dispatchEvent(new FriendRequestEvent(FriendRequestEvent.const_346,_loc4_.requestId));
               }
            }
         }
         this.refresh();
