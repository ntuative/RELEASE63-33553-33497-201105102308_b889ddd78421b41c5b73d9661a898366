package com.sulake.habbo.navigator
{
   ...
   public class RoomPopupCtrl extends PopupCtrl
   {
      ...
      private var var_127:GuestRoomData;
      ...
      public function RoomPopupCtrl(param1:HabboNavigator, param2:int, param3:int)
      {
         ...
         this.var_2727 = new RoomThumbnailRenderer(param1);
         this.var_1167 = new TagRenderer(param1,hideInstantly);
      }
      ...
      override public function refreshContent(param1:IWindowContainer) : void
      {
         ...
         {
            this.var_192 = IWindowContainer(param1.findChildByName("details_container"));
         }
         this.var_192.visible = true;
         Util.hideChildren(this.var_192);
         this.refreshRoomName(this.var_192,this.var_127);
         if(this.var_127.ownerName != "" && this.var_127.ownerName != "-")
         {
         }
         this.refreshOwnerName();
         this.refreshTextWithCaption("roomctg",this.var_192,!!this.var_127.event ? this.getEventCtg(this.var_127.categoryId) : this.getRoomCtg(this.var_127.categoryId));
         this.refreshRoomDesc(this.var_192,this.var_127);
         this.refreshExtraCont();
         this.refreshInfo(this.var_192,"trading_allowed",this.var_127.allowTrading);
         this.refreshInfo(this.var_192,"doormode_doorbell",this.var_127.doorMode == 1);
         this.refreshInfo(this.var_192,"doormode_password",this.var_127.doorMode == 2);
         Util.moveChildrenToColumn(this.var_192,["roomname","roomctg_cont","roomowner_cont","roomdesc","extra_cont","doormode_doorbell","doormode_password","trading_allowed"],0,0);
         this.var_192.height = Util.getLowestPoint(this.var_192);
      }
      
      private function refreshOwnerName() : void
      {
         this.refreshTextWithCaption("roomowner",this.var_192,this.var_127.ownerName);
         var _loc1_:ITextWindow = ITextWindow(this.var_192.findChildByName("roomowner"));
         if(this.var_127.ownerName != "" && this.var_127.ownerName != "-")
         {
      ...
      {
         var _loc1_:IWindowContainer = IWindowContainer(this.var_192.findChildByName("extra_cont"));
         Util.hideChildren(_loc1_);
         this.var_1167.refreshTags(_loc1_,this.var_127.tags);
         this.refreshTextWithCaption("rating",_loc1_,"" + this.var_127.score);
         if(this.var_127.event)
         {
            this.refreshTextWithCaption("startedat",_loc1_,"" + this.var_127.eventCreationTime);
         }
         this.var_2727.refreshThumbnail(_loc1_,this.var_127.thumbnail,false);
      ...
      private function onOwnerName(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         if(_loc3_ != null)
         {
            navigator.mainViewCtrl.startSearch(Tabs.const_275,Tabs.const_289,_loc3_.text);
            this.hideInstantly();
         }
      }
   }
