package com.sulake.habbo.avatar
{
   ...
   public class AvatarAssetDownloadManager extends EventDispatcher
   {
      ...
      private const const_2201:int = 100;
      ...
      public function AvatarAssetDownloadManager(param1:IAssetLibrary, param2:String, param3:String, param4:AvatarStructure)
      {
         ...
         this.var_950 = new Timer(this.const_2201,1);
      ...
      private function addMapLoaderEventListeners() : void
      {
         if(this.var_430)
         {
            this.var_430.addEventListener(AssetLoaderEvent.const_29,this.onConfigurationComplete);
            this.var_430.addEventListener(AssetLoaderEvent.const_49,this.onConfigurationError);
         }
      }
      ...
      private function removeMapLoaderEventListeners() : void
      {
         if(this.var_430)
         {
            this.var_430.removeEventListener(AssetLoaderEvent.const_29,this.onConfigurationComplete);
            this.var_430.removeEventListener(AssetLoaderEvent.const_49,this.onConfigurationError);
         }
      }
      ...
      private function onConfigurationError(param1:Event) : void
      {
         ...
         {
            Logger.log("Could not download figuremap.xml. Retries left: " + this.var_949);
            if(this.var_1454.indexOf("?") > 0)
            {
               _loc2_ = this.var_1454 + "&retry=" + this.var_949;
            }
            else
            {
               _loc2_ = this.var_1454 + "?retry=" + this.var_949;
      ...
      private function generateMap(param1:XML) : void
      {
         ...
         {
            _loc3_ = new AvatarAssetDownloadLibrary(_loc2_.@id,_loc2_.@revision,this.var_2474,this._assets);
            _loc3_.addEventListener(Event.COMPLETE,this.libraryComplete);
            for each(_loc4_ in _loc2_.part)
            {
               _loc5_ = _loc4_.@type + ":" + _loc4_.@id;
               _loc6_ = this.var_1173[_loc5_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
      ...
      public function loadFigureSetData(param1:IAvatarFigureContainer, param2:IAvatarImageListener) : void
      {
         ...
         if(!this.var_1788)
         {
            this.var_1172.push([param1,param2]);
            return;
         }
      ...
      private function libraryComplete(param1:Event) : void
      {
         ...
         while(_loc6_ < this.var_756.length)
         {
            _loc5_ = this.var_756[_loc6_];
            if(_loc5_.libraryName == (param1.target as AvatarAssetDownloadLibrary).libraryName)
            {
               this.var_756.splice(_loc6_,1);
            }
            _loc6_++;
         }
         this.var_950.start();
      ...
      {
         var _loc1_:* = null;
         while(this.var_757.length > 0 && this.var_756.length < this.const_1634)
         {
            _loc1_ = this.var_757[0];
            _loc1_.startDownloading();
            this.var_756.push(this.var_757.shift());
         }
      }
      
      private function addToQueue(param1:AvatarAssetDownloadLibrary) : void
      {
         if(!param1.isReady && this.var_757.indexOf(param1) == -1 && this.var_756.indexOf(param1) == -1)
         {
