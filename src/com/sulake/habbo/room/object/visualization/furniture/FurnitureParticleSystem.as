package com.sulake.habbo.room.object.visualization.furniture
{
   ...
   public class FurnitureParticleSystem
   {
      ...
      private var var_1096:Number = 1.0;
      
      private var var_579:BitmapData;
      
      private var var_712:ColorTransform;
      
      private var var_1358:ColorTransform;
      
      private var var_1639:Matrix;
      ...
      public function FurnitureParticleSystem(param1:AnimatedFurnitureVisualization)
      {
         ...
         this.var_1358 = new ColorTransform();
         this.var_1639 = new Matrix();
      ...
      public function setAnimation(param1:int) : void
      {
         ...
         this.var_97 = this.var_1095[param1];
         this.var_883 = false;
         this.updateCanvas();
      }
      ...
      private function updateCanvas() : void
      {
         ...
         if(this.var_1638 >= 0)
         {
            this.var_243 = this._visualization.getSprite(this.var_1638);
            if(this.var_243 && this.var_243.asset)
            {
               ...
               this.var_2203 = -this.var_243.offsetX;
               this.var_2201 = -this.var_243.offsetY;
      ...
      public function getSpriteYOffset(param1:int, param2:int, param3:int) : int
      {
         ...
         {
            return this.var_97.y * this.var_1096;
         }
         return 0;
      }
      ...
      public function updateAnimation() : void
      {
         ...
         var _loc2_:* = 0;
         var _loc8_:int = 0;
         if(!this.var_883 && this.var_97.hasIgnited)
         {
            this.var_883 = true;
         }
         _loc8_ = this.var_1099 * this.var_1096;
         this.var_97.update();
         if(this.var_883)
         {
            if(this.var_97.roomObjectSpriteId >= 0)
            ...
            {
               this.var_83.draw(this.var_579,this.var_1639,this.var_712,BlendMode.NORMAL,null,false);
            }
            for each(_loc11_ in this.var_97.particles)
            {
               _loc2_ = Number(_loc11_.y);
               _loc3_ = this.var_2203 + (_loc11_.x - _loc11_.z) * 10 / 10 * this.var_1096;
               _loc4_ = this.var_2201 - _loc8_ + (_loc2_ + (_loc11_.x + _loc11_.z) / 2) * 10 / 10 * this.var_1096;
               ...
               if(_loc6_)
               {
                  ...
                  if(_loc11_.fade && _loc11_.alphaMultiplier < 1)
                  {
                     ...
                     this.var_1358.alphaMultiplier = _loc11_.alphaMultiplier;
                     this.var_83.draw(_loc7_,this._translationMatrix,this.var_1358,BlendMode.NORMAL,null,false);
                  }
                  else
                  {
      ...
      public function parseData(param1:XML) : void
      {
         ...
         var _loc23_:* = null;
         var _loc24_:* = null;
         this.var_2202 = parseInt(param1.@size);
         this.var_1638 = !!param1.hasOwnProperty("@canvas_id") ? int(parseInt(param1.@canvas_id)) : -1;
         this.var_1099 = !!param1.hasOwnProperty("@offset_y") ? int(parseInt(param1.@offset_y)) : 10;
         this.var_1096 = this.var_2202 / 64;
         this._blend = !!param1.hasOwnProperty("@blend") ? Number(Number(param1.@blend)) : Number(1);
         this._blend = Math.min(this._blend,1);
         this.var_712.alphaMultiplier = this._blend;
